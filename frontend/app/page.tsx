    ...
    const fetchTelemetry = async () => {
      ...
      try {
        const [statsRes, dueRes] = await Promise.all([
          fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}`/api/progress/summary", { headers }),
          fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}`/api/progress/due", { headers })
        ]);
