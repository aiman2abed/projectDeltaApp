...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    // Pass the names into the Supabase user_metadata
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center lg:items-start p-8 sm:p-12 lg:pt-28 bg-slate-900 relative overflow-hidden">
        ...
        <div className="w-full max-w-md relative z-10">
          <div className="lg:hidden mb-8 text-center">
            ...
            <p className="text-xs font-bold tracking-[0.18em] text-indigo-400 uppercase">Spirelay</p>
          </div>
          ...
          <div className="rounded-3xl border border-white/10 bg-white/5 backdrop-blur-xl p-8 sm:p-10 shadow-[0_20px_80px_rgba(0,0,0,0.35)]">
            <div className="mb-8 text-center lg:text-left">
              <p className="text-xs font-bold tracking-[0.2em] text-emerald-300 uppercase mb-3">Join Spirelay</p>
            ...
            <form className="space-y-5" onSubmit={handleSignup}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-slate-200 mb-1.5">First Name</label>
                  <input
                    type="text"
                    required
                    value={firstName}
                    onChange={(e) => setFirstName(e.target.value)}
                    className="w-full px-4 py-3 bg-white/10 border border-white/10 text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/30 transition-all placeholder:text-slate-400"
                    placeholder="Ada"
                  />
                </div>
                <div>
                  <label className="block text-sm font-bold text-slate-200 mb-1.5">Last Name</label>
                  <input
                    type="text"
                    required
                    value={lastName}
              ...
              <div>
                <label className="block text-sm font-bold text-slate-200 mb-1.5">Password</label>
                <input
                  type="password"
                  required
                  minLength={6}
            ...
            <div className="mt-6 text-center text-sm">
              <p className="text-slate-300 font-medium">
                ...
                <Link href="/login" className="text-indigo-300 font-bold hover:text-emerald-300 transition-colors">
                  Sign in here
                </Link>
              </p>
            </div>
