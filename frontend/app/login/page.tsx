...
export default function LoginPage() {
  ...
  const handleRecoverKeys = async (e: React.MouseEvent) => {
    e.preventDefault();
    setError(null);
    setMessage(null);

    if (!email) {
      setError("Please enter your Email Address to initiate key recovery.");
      return;
    }

    setLoading(true);
    
    // Sends the reset email and redirects them to the update page once they click the link
    ...
    if (error) {
      ...
      setMessage("Recovery sequence initiated. Check your inbox for the reset link.");
    }
          ...
          <div className="flex justify-center mb-4">
            <div className="drop-shadow-[0_0_24px_rgba(99,102,241,0.18)]">
              <div className="w-72 xl:w-[24rem] mx-auto">
                <Image src="/spirelay_logo_noBg.png" alt="Spirelay Logo" width={900} height={900} className="object-contain w-full h-auto" priority />
              </div>
            </div>
          </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center lg:items-start p-8 sm:p-12 lg:pt-32 bg-slate-900 relative overflow-hidden">
        ...
        <div className="w-full max-w-md relative z-10">
          <div className="lg:hidden mb-8 text-center">
            ...
            <p className="text-xs font-bold tracking-[0.18em] text-sky-400 uppercase">Spirelay</p>
          </div>
          ...
          <div className="rounded-3xl border border-white/10 bg-white/5 backdrop-blur-xl p-8 sm:p-10 shadow-[0_20px_80px_rgba(0,0,0,0.35)]">
            <div className="mb-10 text-center lg:text-left">
              <p className="text-xs font-bold tracking-[0.2em] text-sky-400 uppercase mb-3">Secure Portal</p>
              <h2 className="text-3xl font-extrabold text-white tracking-tight">Operator Login</h2>
            </div>

            <form className="space-y-6" onSubmit={handleLogin}>
              <div>
                <label className="block text-sm font-bold text-slate-200 mb-1.5">Email Address</label>
                <input
                  type="email"
                  required
                  value={email}
              ...
              <div>
                <div className="flex justify-between items-center mb-1.5">
                  ...
                  <button 
                    ...
                    className="text-xs font-bold text-slate-400 hover:text-sky-400 transition-colors bg-transparent border-none cursor-pointer"
                  >
                    Recover keys?
                  </button>
                </div>
            ...
            <div className="mt-8 text-center text-sm">
              <p className="text-slate-300 font-medium">
                ...
                <Link href="/signup" className="text-sky-400 font-bold hover:text-sky-300 transition-colors">
                  sign up here
                </Link>
              </p>
            </div>
